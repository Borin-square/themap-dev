  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Splitta HTML e CSS custom sui marker
        const CSS_START = "<!--__PG_CSS_START__-->";
        const CSS_END = "<!--__PG_CSS_END__-->";
        let htmlPart = fullText;
        let cssPart: string | null = null;
        const startIdx = fullText.indexOf(CSS_START);
        if (startIdx !== -1) {
          const endIdx = fullText.indexOf(CSS_END, startIdx);
          const cssRaw = endIdx !== -1
            ? fullText.slice(startIdx + CSS_START.length, endIdx)
            : fullText.slice(startIdx + CSS_START.length);
          cssPart = cssRaw.trim() || null;
          htmlPart = fullText.slice(0, startIdx);
        }

        // Pulizia HTML: rimuovi eventuali code fence residui
        ...
        await svc.from("pg_page_versions")
          .update({ html_output: cleanedHtml, css_output: cssPart })
          .eq("id", version.id);
